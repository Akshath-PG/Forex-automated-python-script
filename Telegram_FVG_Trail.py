...
def check_daily_loss():
    ...
    if loss_pct >= DAILY_LOSS_LIMIT:
        ...
        admin_only(
            ...
            f"\n"
            f"─────────────────────────────\n"
            f"Bot has PAUSED for today.\n"
            f"Use /resume tomorrow. 🛑"
        )
        return True
    return False
    ...
    for pos in positions:
        ...
        if targets and (current_sl == 0.0 or current_sl is None) and (tp == 0.0 or tp is None):
            ...
            if apply_sltp(ticket, real_sl, real_tp):
                admin_only(
                    f"⏱  HOLD COMPLETE — SL/TP SET\n"
                    f"─────────────────────────────\n"
                    f"🎫 Ticket  :  #{ticket}\n"
                    f"🛡 SL Set  :  {real_sl}\n"
                    f"💎 TP Set  :  {real_tp}\n"
                    f"─────────────────────────────\n"
                    f"Trade is now fully protected ✅"
                )
                with state_lock:
...
def market_scanner():
    ...
    broadcast(
        f"╔══════════════════════════╗\n"
        f"║   ⚡ FVG SCALPER ONLINE   ║\n"
        f"╚══════════════════════════╝\n\n"
        f"📍 Symbol   : XAUUSD\n"
        f"⏱ Timeframe : M5\n"
        f"🔍 Mode     : Predictive (3 Triggers)\n"
        f"💰 Risk     : {RISK_PERCENT*100}% per trade\n"
        f"📊 Max Trades: {MAX_TRADES}\n"
        f"─────────────────────────────\n"
        f"Bot is scanning. You will be\n"
        f"notified when entries fire. 🚀"
    )

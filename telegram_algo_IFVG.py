...
def start_command(message):
    ...
    bot.reply_to(message,
        ...
        "/stats         — win rate & P&L\n"
        "/closeall      — close all live trades NOW (admin)\n"
        "/closepending  — cancel all unfilled limit orders (admin)\n"
...
def cmd_close_all(message):
    ...
    positions = mt5.positions_get(symbol=SYMBOL)
    if not positions:
        bot.reply_to(message, "📭 No open trades to close.")
        return
    ...
    for pos in positions:
        ...
        if res and res.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            with state_lock:
                ...
                if "breakeven_tickets" in shared:
                    shared["breakeven_tickets"].discard(pos.ticket)
            log_trade_db(pos.ticket, SYMBOL, "CLOSED", pos.price_open, pos.sl, pos.tp, pos.volume, pnl, "MANUAL")
            log_trade_csv(pos.ticket, SYMBOL, "CLOSED", pos.price_open, pos.sl, pos.tp, pos.volume, pnl, "MANUAL")
        else:
            failed.append(pos.ticket)

    lines = [f"🔴 CLOSE ALL — {len(closed)} closed, {len(failed)} failed\n"]
    if closed:
        lines.append(f"✅ Closed  : {', '.join(f'#{t}' for t in closed)}")
    if failed:
        lines.append(f"❌ Failed  : {', '.join(f'#{t}' for t in failed)}")
...
def cmd_cancel_pending(message):
    ...
    if not pending_orders:
        bot.reply_to(message, "📭 No pending orders to cancel.")
        return
    ...
    for order in pending_orders:
        ...
        if res and res.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            with state_lock:
                shared["active_tickets"] = [t for t in shared["active_tickets"] if t != order.ticket]
        else:
            failed.append(order.ticket)

    lines = [f"🗑 CANCEL PENDING — {len(cancelled)} cancelled, {len(failed)} failed\n"]
    if cancelled:

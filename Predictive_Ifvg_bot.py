...
CONTRACT_SIZE  = 100
RR_RATIO       = 2.0
SCAN_INTERVAL  = 15           # faster scan — M1 moves fast

MIN_HOLD_SECONDS        = 180
DAILY_LOSS_LIMIT        = 0.02
MAX_TRADES              = 3
MIN_IFVG_GAP            = 8.0   # was 15.0 — too large for M1 candles, rarely triggered
MOMENTUM_IFVG_THRESHOLD = 5     # was 15 — unreachably high, now realistic
...
APPROACH_PROXIMITY   = 20.0   # was 12.0 — wider net for IFVG approach trigger
ENTRY_INSIDE_PCT     = 0.20
PENDING_EXPIRY_MIN   = 15
STRUCTURE_LOOKBACK   = 30
STRUCTURE_SWING_N    = 5
IFVG_LOOKBACK_BARS   = 500    # how far back to look for IFVG formation (was limited to 200)
...
def get_stats(trigger=None):
    """
    WIN       = pnl > 0  (TP hit or trailing SL with profit)
    BREAKEVEN = pnl == 0 (SL at entry — counts as WIN for win rate)
...
def trend_allows(side, ifvg_count=0):
    """M15 + H4 must agree. Momentum override: if ifvg_count >= threshold, M15 bypassed."""
...
def detect_ifvg(df, fvgs):
    """
    Inverted FVGs — gaps that price has fully closed back through.
    An IFVG is a high-probability re-test zone: price broke through once,
    now returning to that zone is a strong signal.
    Only keeps IFVGs where price is currently AWAY from the zone
    (i.e. approaching it, not already inside it).
    """
    ifvgs = []
    current_price = df.iloc[-1].close
    ...
    for gap in fvgs:
        if (gap["high"] - gap["low"]) < MIN_IFVG_GAP:
            continue
        # Look forward from gap formation to find where price crossed through
        for i in range(gap["index"], len(df)):
            price = df.iloc[i].close

            if gap["type"] == "bullish" and price < gap["low"]:
                # Price broke below a bullish FVG → this is now a bearish IFVG
                # Only valid if current price is ABOVE the zone (approaching from above)
            ...
            if gap["type"] == "bearish" and price > gap["high"]:
                # Price broke above a bearish FVG → this is now a bullish IFVG
                # Only valid if current price is BELOW the zone (approaching from below)
...
def trigger1_ifvg_approach(df_m1, fvgs, ifvgs, current_price, ifvg_count):
    """
    TRIGGER 1 — Price Approaching IFVG Zone
    Detects when price is within APPROACH_PROXIMITY of an existing IFVG.
    M15 + H4 filter applies (with momentum override — M15 bypassed if enough FVGs).
    """
    signals = []
    for gap in ifvgs:
        gap_id   = gap["gap_id"]
        ...
        if gap["type"] == "bullish_ifvg":
            ...
            if 0 < dist <= APPROACH_PROXIMITY:
                ...
                if trend_allows("BUY", ifvg_count):
            ...
            if 0 < dist <= APPROACH_PROXIMITY:
                ...
                if trend_allows("SELL", ifvg_count):
...
def trigger2_inversion_forming(df_m1, fvgs, ifvg_count):
    """
    TRIGGER 2 — Live Inversion Forming on M1
    Price is actively crossing back through a FVG gap right now.
    M15 bypassed — only H4 must agree (or momentum override).
    """
    signals = []
    if len(df_m1) < 3:
        return signals
    ...
    for gap in fvgs:
        ...
        if gap["type"] == "bullish":
            ...
            if prev_price >= gap["low"] and current_price < gap["low"]:
                if h4 == "bearish" or ifvg_count >= MOMENTUM_IFVG_THRESHOLD:
                    near_edge = gap["low"]
                    far_edge  = gap["high"]
                    entry = near_edge + (far_edge - near_edge) * ENTRY_INSIDE_PCT
                    sl    = near_edge + gap_size * 0.3
            ...
            if prev_price <= gap["high"] and current_price > gap["high"]:
                if h4 == "bullish" or ifvg_count >= MOMENTUM_IFVG_THRESHOLD:
                    near_edge = gap["high"]
                    far_edge  = gap["low"]
                    entry = near_edge + (far_edge - near_edge) * ENTRY_INSIDE_PCT
                    sl    = near_edge - gap_size * 0.3
...
def trigger3_structure_break_m5(ifvg_count):
    """
    TRIGGER 3 — Structure Break on M5
    Uses M5 for higher-quality swing detection.
    H4 must agree (M15 bypassed — structure breaks are directional).
    """
    signals = []
    ...
    for i in range(n, len(window) - n - 2):
        ...
        if is_swing_high and current_close > candle.high:
            ...
            if not fired and (h4 == "bullish" or ifvg_count >= MOMENTUM_IFVG_THRESHOLD):
        ...
        if is_swing_low and current_close < candle.low:
            ...
            with state_lock:
                fired = gap_id in shared["fired_gaps"]
            if not fired and (h4 == "bearish" or ifvg_count >= MOMENTUM_IFVG_THRESHOLD):
...
def manage_open_trades():
    ...
    for pos in positions:
        ...
        if open_time is None:
            # Give monitor_predictive_orders() 5 seconds to link it via deal history
        ...
        if pos.type == mt5.ORDER_TYPE_BUY:
            ...
            if new_sl > normalize_price(SYMBOL, current_sl):
                ...
                if locked_msg:
                    admin_only(f"🔒 BUY #{ticket}: SL moved — {locked_msg}")
            ...
            if new_sl < normalize_price(SYMBOL, current_sl):
                ...
                with state_lock:
                    shared["breakeven_tickets"].add(ticket)
                if locked_msg:
                    admin_only(f"🔒 SELL #{ticket}: SL moved — {locked_msg}")
...
def cmd_status(message):
    ...
    if not ensure_mt5():
        bot.reply_to(message,
            "❌ *MT5 Not Connected*\n\nPlease make sure MetaTrader 5 is open and running.",
            parse_mode="Markdown")
        return
...
def cmd_buy(message):
    if not admin_guard(message):
        return
    bot.reply_to(message, "⏳ *Placing manual BUY order...*\n_0.1 lot on XAUUSD_", parse_mode="Markdown")
...
def cmd_sell(message):
    if not admin_guard(message):
        return
    bot.reply_to(message, "⏳ *Placing manual SELL order...*\n_0.1 lot on XAUUSD_", parse_mode="Markdown")
...
def cmd_debug(message):
    ...
    try:
        if not ensure_mt5():
            bot.send_message(CHAT_ID,
                "❌ *MT5 Not Connected*\n\nThe bot cannot reach MT5.\n_Make sure MT5 is open and logged in._",
                parse_mode="Markdown")
            return
        df_m1 = load_mt5(SYMBOL, TIMEFRAME_M1, IFVG_LOOKBACK_BARS)
        if df_m1 is None:
            bot.send_message(CHAT_ID,
                "❌ *MT5 Data Unavailable*\n\nFailed to fetch M1 candle data for XAUUSD.",
                parse_mode="Markdown")
            return
        fvgs        = detect_fvg(df_m1)
        ifvgs       = detect_ifvg(df_m1, fvgs)
        ifvg_count  = get_fresh_fvg_count(df_m1, fvgs, lookback=100)
        m15         = get_trend(TIMEFRAME_M15)
        h4          = get_trend(TIMEFRAME_H4)
        momentum    = ifvg_count >= MOMENTUM_IFVG_THRESHOLD
        ...
        bot.send_message(CHAT_ID,
            ...
            f"📡 *Valid IFVGs detected:* {len(ifvgs)}\n"
            f"📊 *Fresh FVGs (last 100 bars):* {ifvg_count}\n"
            f"🚀 *Momentum Mode:* {'YES ✅' if momentum else f'NO — need {MOMENTUM_IFVG_THRESHOLD}, have {ifvg_count}'}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📰 *News Window:* {'🚫 YES — trading paused' if news_active else '✅ Clear'}\n"
            f"⚠️ *Daily Loss Hit:* {'YES 🚨' if loss_hit else 'NO ✅'}\n",
            parse_mode="Markdown")
    except Exception as e:
        bot.send_message(CHAT_ID, f"❌ *Diagnostics Error*\n\n`{e}`", parse_mode="Markdown")
...
def market_scanner():
    ...
    broadcast("⚡ Akshath IFVG Predictive Bot Online — XAUUSD M1")
    scan_count = 0
    ...
    while True:
        try:
            ...
            t1 = trigger1_ifvg_approach(df_m1, fvgs, ifvgs, current_price, ifvg_count)
            signals.extend(t1)

            t2 = trigger2_inversion_forming(df_m1, fvgs, ifvg_count)
            signals.extend(t2)

            t3 = trigger3_structure_break_m5(ifvg_count)   # re-enabled
            signals.extend(t3)
            ...
            if signals:
                ...
                print(f"[Scan #{scan_count}] No triggers | IFVGs={len(ifvgs)} "
                      f"FreshFVGs={ifvg_count} | M15={m15.upper()} H4={h4.upper()} "
                      f"{'MOMENTUM 🚀' if momentum else ''} | Price={round(current_price,2)}")
